...
5. **Before the first `docker compose up`, create a top-level `/opt/app/.env`** (next to `docker-compose.yml`, distinct from `hermes_home/.env`). This is required — `env_file: ./hermes_home/.env` only injects vars into the `hermes` container's own runtime, it does **not** feed Compose's `${...}` interpolation used elsewhere in `docker-compose.yml`. Without this file, `docker compose up` prints `variable is not set, Defaulting to a blank string` and silently breaks things: `group_add: ["${DOCKER_GID}"]` ends up empty (hermes's non-root user never joins the host's `docker` group → the sandbox/terminal tool can't use the bind-mounted socket), and `HERMES_DASHBOARD_USERNAME`/`HERMES_DASHBOARD_PASSWORD` end up empty (web-admin can't log in to hermes's dashboard → every `/api/skills*` request 401s). Populate it with:
   ```
   DOCKER_GID=$(getent group docker | cut -d: -f3)
   HERMES_DASHBOARD_BASIC_AUTH_USERNAME=<same value as in hermes_home/.env>
   HERMES_DASHBOARD_BASIC_AUTH_PASSWORD=<the PLAINTEXT password — hermes_home/.env only holds its scrypt hash (HERMES_DASHBOARD_BASIC_AUTH_PASSWORD_HASH), which can't be reversed to log in with. web-admin needs the real password to POST /auth/password-login.>
   ```
   `chmod 600` it, same as `hermes_home/.env`.

   Note: the dashboard (`hermes_cli/web_server.py`, port 9119) is a separate service from the messaging/session API (`gateway/platforms/api_server.py`, port 8642, protected by `API_SERVER_KEY`) — don't confuse the two. `/api/skills*` only exists on the dashboard, and it authenticates via a username/password login (`/auth/password-login`) that issues a session cookie, not a static bearer token. See `hermes_home/.env` for the `HERMES_DASHBOARD=1` / `HERMES_DASHBOARD_BASIC_AUTH_*` variables that enable it.
